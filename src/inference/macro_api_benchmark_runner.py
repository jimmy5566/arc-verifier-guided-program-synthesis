...
def _worker(worker_id: int, model_path: str, frozen: Mapping[str, Any], cases: Queue, results: Queue, ready: Queue, start: Event, checkpoint_root: str) -> None:
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    parsed_program = None
                record = {"case_id": case_id, "worker_id": worker_id, "gpu_id": worker_id, "prompt_tokens": generated.prompt_tokens, "completion_tokens": generated.completion_tokens, "latency_seconds": generated.elapsed_seconds, "raw_response": generated.text, "parsed_program": parsed_program, "score": score, "provider_status": "SUCCESS"}
            except Exception as exc:
                record = {"case_id": case_id, "worker_id": worker_id, "gpu_id": worker_id, "prompt_tokens": 0, "completion_tokens": 0, "latency_seconds": time.perf_counter() - started, "raw_response": None, "parsed_program": None, "score": score_response(case, None), "provider_status": f"FAILED:{type(exc).__name__}", "provider_error": str(exc)}
        ...
        ready.put({"event": "WORKER_FAILED", "worker_id": worker_id, "gpu_id": worker_id, "error": f"{type(exc).__name__}: {exc}"})
...
def summarize(records: list[Mapping[str, Any]], frozen: Mapping[str, Any], benchmark: Mapping[str, Any], worker_reports: list[Mapping[str, Any]], warmup: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {"experiment_id": "MACRO_API_COMPREHENSION_BENCHMARK_V1", "status": "COMPLETE_FROZEN_ONE_RUN", "new_arc_inference": False, "arc_data_used": False, "arc_solutions_used": False, "gpu_used": True, "model": frozen["model"], "frozen_config_sha256": hashlib.sha256(json.dumps(frozen, sort_keys=True, separators=(",", ":")).encode()).hexdigest(), "benchmark_hash": benchmark["benchmark_hash"], "registry_hash": benchmark["registry_hash"], "schema_hash": benchmark["schema_hash"], "prompt_version": frozen["prompt_version"], "sampling": frozen["sampling"], "candidate_budget": 1, "max_new_tokens": frozen["generation"]["max_new_tokens"], "case_count": total, "worker_to_gpu_mapping": {str(item["worker_id"]): item["gpu_id"] for item in worker_reports}, "model_warmup": warmup, "worker_reports": worker_reports, "funnel": funnel, "rates": {field: value / total for field, value in funnel.items()}, "category_metrics": categories, "canonical_macro_sequence_matches": sum(bool(item["score"]["canonical_macro_sequence_match"]) for item in records), "instruction_constraints_valid": sum(bool(item["score"]["instruction_constraints_valid"]) for item in records), "top_failure_types": failures.most_common(10), "go_decision": decision, "recommended_next_experiment": "ORACLE_LADDER" if decision == "GO_API_LEARNED" else "API_REPRESENTATION_ABLATION", "raw_response_policy": "Raw responses and parsed programs are retained only in ignored local/Kaggle checkpoint artifacts; this aggregate has no raw completion content."}
...
def run_once(*, benchmark: Mapping[str, Any], frozen: Mapping[str, Any], model_path: Path, output: Path, checkpoints_root: Path) -> dict[str, Any]:
    ...
    result = summarize(records, frozen, benchmark, worker_reports, warmup) | {"hardware": hardware.to_dict(), "completed_at_utc": _utc_now()}
