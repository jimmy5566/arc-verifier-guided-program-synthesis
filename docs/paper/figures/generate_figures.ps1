...
function Draw-BarChart($filename, $title, $labels, $series, [int]$maximum, $ylabel) {
...
Draw-BarChart 'frozen30_recall_selection.png' 'Candidate recall does not directly determine selected success' @('A baseline','C search','D search + provenance') @(@{name='Any-of-K (diagnostic)';values=@(19,22,22)},@{name='Top-1';values=@(9,8,9)},@{name='Two attempts';values=@(11,9,13)}) 25 'Exact tasks (N=25)'
# Source: local/private artifacts/untouched60_scored.json. Frozen before inference and target access.
Draw-BarChart 'untouched60_ab.png' 'One untouched frozen A/B cohort (paired two-attempt p = 0.375)' @('Any-of-K (diagnostic)','Top-1','Two attempts') @(@{name='A baseline';values=@(30,18,18)},@{name='B provenance-aware';values=@(30,16,21)}) 60 'Exact tasks (N=60)'
# Source: local/private artifacts/untouched60_dynamic_queue_simulation.json. CPU-only replay.
Draw-BarChart 'scheduler_replay.png' 'Four-worker historical replay: unchanged inference semantics' @('Static','Dynamic queue') @(@{name='Modeled makespan (seconds)';values=@(7558,5564)}) 8500 'Modeled makespan (seconds)'
