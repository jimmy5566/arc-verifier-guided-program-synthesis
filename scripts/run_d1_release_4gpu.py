...
def validate_live_config(config: Mapping[str, Any]) -> None:
    ...
    if set(config) < required: raise ReleaseContractError(f"release config missing {sorted(required - set(config))}")
    expected = str(config["model_identity"].get("checkpoint_sha256", ""))
    if len(expected) != 64 or any(character not in "0123456789abcdef" for character in expected.lower()):
        raise ReleaseContractError("model checkpoint SHA256 is not pinned")
...
def _selected_generation(*, model: Any, tokenizer: Any, task: Any, config: Mapping[str, Any], tags: tuple[str, ...]) -> tuple[list[dict[str, Any]], int, float]:
    ...
    all_views = {item.geometry: item for item in bounded_native_augmentations()}
    if set(tags) - set(all_views):
        raise ReleaseContractError(f"unknown frozen portfolio views: {sorted(set(tags) - set(all_views))}")
    views = tuple(all_views[tag] for tag in tags)
    ...
    for view_index, (view, augmented) in enumerate(zip(views, transformed, strict=True)):
        ...
        for test_index in range(len(task.test)):
            ...
            torch.manual_seed(task_seed(task.task_id, int(config["seed"]), f"D1:{config['ttt_steps']}:{view.geometry}:{test_index}")); torch.cuda.manual_seed_all(torch.initial_seed())
...
def _live_worker(worker_id: int, work: Any, events: Any, ready: Any, start: Any, paths: dict[str, str], release_config: dict[str, Any], manifest: dict[str, Any]) -> None:
        ...
        while True:
            ...
            try:
                ...
                for source, recipe in (("TTT24", recipe24), ("TTT48", recipe48)):
                    ...
                    if not candidates: raise RuntimeError(f"{source} produced an empty candidate pool")
                    ...
                    sources[source] = {"candidates": candidates, "per_output_evidence": evidence}; source_metrics[source] = {"recipe_sha256": hashlib.sha256(json.dumps(recipe, sort_keys=True, separators=(",", ":")).encode()).hexdigest(), "ttt": ttt, "invalid_candidate_count": invalid, "generation_seconds": generation_seconds, "adapter_reset_success": reset_ok}
...
def run_live(challenge_path: Path, release_config: dict[str, Any], model_path: Path, native_config: Path, checkpoint_dir: Path, *, resume: bool) -> dict[str, Any]:
    ...
    context = get_context("spawn"); work, events, ready, start = context.Queue(), context.Queue(), context.Queue(), context.Event(); processes = []; failures = []; loads = []; started = time.perf_counter()
    ...
    try:
        for worker_id in range(4):
            process = context.Process(target=_live_worker, args=(worker_id, work, events, ready, start, paths, release_config, manifest)); process.start(); processes.append(process)
        ...
        start.set()
        while len(records) + len(failures) < len(manifest["task_ids"]):
            try: event = events.get(timeout=120)
            except queue.Empty:
                dead = [process.pid for process in processes if process.exitcode not in (None, 0)]
                if dead: raise RuntimeError(f"D1 worker died: {dead}")
                continue
        ...
        start.set()
        for process in processes: process.join(timeout=120)
        for process in processes:
            if process.is_alive(): process.terminate()
    return {"schema_version": manifest["schema_version"], "release_identity": manifest["release_identity"], "manifest": manifest, "records": {task_id: records[task_id] for task_id in manifest["task_ids"]}, "worker_count": 4, "worker_model_loads": loads, "runtime_seconds": time.perf_counter() - started, "solutions_opened": False}
