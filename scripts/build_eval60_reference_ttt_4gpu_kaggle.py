...
def _notebook(dataset_slug: str) -> str:
    return "\n".join([
        ...
        'inputs=Path("/kaggle/input")', 'archive=next(inputs.rglob("ARC2.zip"))', 'source=Path("/kaggle/working/eval60_reference_ttt_source")', 'shutil.unpack_archive(archive,source)', 'root=next(source.rglob("run_eval60_reference_ttt_4gpu.py")).parents[1]', 'frozen=next(source.rglob("eval60_reference_ttt_4gpu_manifest.json")).parent',
