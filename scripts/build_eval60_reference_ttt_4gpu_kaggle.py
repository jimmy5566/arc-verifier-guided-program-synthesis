...
def _notebook(dataset_slug: str) -> str:
    return "\n".join([
        ...
        'inputs=Path("/kaggle/input")', 'runner=next(inputs.rglob("run_eval60_reference_ttt_4gpu.py"),None)', 'source=inputs if runner is not None else Path("/kaggle/working/eval60_reference_ttt_source")', 'if runner is None: shutil.unpack_archive(next(inputs.rglob("ARC2.zip")),source)', 'root=(runner if runner is not None else next(source.rglob("run_eval60_reference_ttt_4gpu.py"))).parents[1]', 'frozen=next((inputs if runner is not None else source).rglob("eval60_reference_ttt_4gpu_manifest.json")).parent',
