...
def main() -> None:
    parser = argparse.ArgumentParser(); parser.add_argument("--predictions", type=Path, required=True); parser.add_argument("--gold", type=Path, required=True); args = parser.parse_args(); started = perf_counter()
    ...
    family_metrics = {family: {"tasks": len(items), "semantic_success": sum(bool(item["semantic_success"]) for item in items), "primary_family_correct": sum(bool(item["primary_family_correct"]) for item in items)} for family, items in sorted(families.items())}
    ...
    with (ROOT / "experiments/experiments.csv").open("a", encoding="utf-8", newline="") as handle:
        ...
        writer.writerow({"experiment_id": "GRID_SEMANTIC_RECOGNITION_V1", "date": str(date.today()), "git_commit": "pending", "solver": "Qwen semantic recognizer", "representation": "ARCSEMANTICIRV1", "search_method": "top-1 greedy recognition only", "llm_model": "qwen-lm/qwen-3/Transformers/8b/1", "candidate_budget": 1, "validation_split": "development_only_30", "tasks_solved": sum(bool(item["semantic_success"]) for item in scored.values()), "accuracy": metrics["semantic_ir_success"], "runtime_seconds": frozen.get("runtime_seconds", 0), "gpu_hours": 0, "notes": json.dumps({"recognition_status": status, "downstream_used": False}, sort_keys=True)})
