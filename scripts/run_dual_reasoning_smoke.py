...
def _model_role(branch: str) -> str:
    return "native" if branch=="native" else "soar"
...
def _induction_transformers(args: argparse.Namespace,tasks:dict[str,ARCTask],path:Path)->dict:
    import torch
    ...
    peak=gpu_memory_mb();del model,tokenizer;unload=unload_model(None);return {"branch":"induction","status":"BRANCH_PREDICTIONS_FROZEN","model_path":str(path),"loader_backend":"transformers","records":records,"peak_vram_mb":peak,"gpu_after_unload_mb":unload}

def _llama_server_binary(input_root: Path, destination: Path) -> Path:
    attached=sorted(path for path in input_root.rglob("llama-server") if path.is_file())
    installed=shutil.which("llama-server")
    choices=attached+([Path(installed)] if installed else [])
    if len(choices)!=1: raise RuntimeError(f"requires one attached or installed llama-server for SOAR GGUF, found={len(choices)}")
    destination.mkdir(parents=True,exist_ok=True);target=destination/"llama-server";shutil.copy2(choices[0],target);target.chmod(target.stat().st_mode|0o111);return target

def _induction_gguf(args: argparse.Namespace,tasks:dict[str,ARCTask],path:Path)->dict:
    from inference.llama_cpp_backend import LlamaCppServer
    binary=_llama_server_binary(args.input_root,args.output.parent/"soar_llama_runtime");log=args.output.parent/"soar_llama_runtime"/"llama-server.log";server=LlamaCppServer(binary=binary,model=path,endpoint="http://127.0.0.1:18081",gpu_id=0,context_window=8192,log_path=log);server.start();records={}
    def generate(prompt:str,seed:int)->str:
        response=server.chat(prompt=prompt,max_tokens=args.soar_max_new_tokens,temperature=0,top_p=1,seed=seed);return str(response.get("choices",[{}])[0].get("message",{}).get("content", ""))
    try:
        for task_id in args.task_ids:
            task=tasks[task_id];started=perf_counter();pairs=[(item.input.to_list(),item.output.to_list())for item in task.train];raw=generate(soar_prompt(pairs),20260914);program=extract_program(raw);verification=verify_program(program,pairs) if program else {"program_valid":False,"train_pass_count":0,"train_pair_count":len(pairs),"all_train_exact":False};test=execute_program(program,task.test[0].input.to_list()) if program and verification["all_train_exact"] else {"ok":False,"status":"NOT_TRAIN_VERIFIED"};loo=[]
            if len(task.train)==3:
                for held in range(3):
                    reduced,expected=_subset_task(task,held);pairs_loo=[(item.input.to_list(),item.output.to_list())for item in reduced.train];loo_program=extract_program(generate(soar_prompt(pairs_loo),20260914+held));result=execute_program(loo_program,expected.input.to_list()) if loo_program else {"ok":False};loo.append(bool(result.get("ok") and result.get("grid")==expected.output.to_list()))
            records[task_id]={"raw_response":raw,"program_text":program,"verification":verification,"test_execution":test,"loo_successes":sum(loo),"loo_total":len(loo),"loo_score":sum(loo)/len(loo) if loo else None,"runtime_seconds":perf_counter()-started}
        peak=gpu_memory_mb()
    finally: server.stop()
    unload=unload_model(None);return {"branch":"induction","status":"BRANCH_PREDICTIONS_FROZEN","model_path":str(path),"model_size_bytes":path.stat().st_size,"loader_backend":"llama.cpp llama-server","records":records,"peak_vram_mb":peak,"gpu_after_unload_mb":unload,"server_log":str(log)}

def _induction(args: argparse.Namespace,tasks:dict[str,ARCTask],path:Path)->dict:
    return _induction_gguf(args,tasks,path) if path.suffix.lower()==".gguf" else _induction_transformers(args,tasks,path)

def main()->None:
 p=argparse.ArgumentParser();p.add_argument("--branch",choices=("native","induction"),required=True);p.add_argument("--models-root",type=Path,default=Path("/kaggle/input/models"));p.add_argument("--input-root",type=Path,default=Path("/kaggle/input"));p.add_argument("--challenge-path",type=Path,required=True);p.add_argument("--native-config-dir",type=Path,required=True);p.add_argument("--output",type=Path,required=True);p.add_argument("--native-max-new-tokens",type=int,default=256);p.add_argument("--soar-max-new-tokens",type=int,default=768);args=p.parse_args();args.task_ids=TASK_IDS
 if args.output.exists():raise FileExistsError("refusing to overwrite branch artifact")
 tasks=load_dataset(args.challenge_path);required_role=_model_role(args.branch);paths=discover_models(args.models_root,required=(required_role,),input_root=args.input_root);result=_native(args,tasks,paths["native"])if args.branch=="native"else _induction(args,tasks,paths["soar"]);result["task_ids"]=list(args.task_ids);result["discovered_models"]={name:str(value)for name,value in paths.items()};args.output.parent.mkdir(parents=True,exist_ok=True);args.output.write_text(json.dumps(result,indent=2)+"\n",encoding="utf-8");print(json.dumps({"branch":args.branch,"status":result["status"],"tasks":len(result["records"])},sort_keys=True))
