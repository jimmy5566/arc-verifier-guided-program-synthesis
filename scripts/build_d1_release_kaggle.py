...
def notebook(source_archive: str, source_sha256: str, config_name: str, *, smoke_task_ids: tuple[str, ...] = ()) -> dict[str, Any]:
    ...
    if smoke_task_ids:
        ...
        main_index = lines.index("def _d1_main():")
        lines[main_index + 1:main_index + 5] = [
            '    work=Path("/kaggle/working"); mounted_eval=Path("/kaggle/input/competitions/arc-prize-2026-arc-agi-2/arc-agi_evaluation_challenges.json")',
            '    eval_tasks=json.loads(mounted_eval.read_text()); smoke_ids=' + repr(smoke_task_ids) + '; assert all(task_id in eval_tasks for task_id in smoke_ids)',
            '    challenge=work/"d1_smoke_challenges.json"; challenge.write_text(json.dumps({task_id:eval_tasks[task_id] for task_id in smoke_ids},sort_keys=True))',
            '    submission=work/"artifacts"/"d1_release"/"smoke_submission.json"; fast_provenance=work/"artifacts"/"d1_release"/"FAST_SAVE_PROVENANCE.json"',
            '    return _d1_route("true",lambda:_d1_fast_save(challenge,submission,fast_provenance),lambda:_d1_full_rerun(challenge,submission,fast_provenance))',
        ]
        config_line = '    cfg=json.loads(config.read_text())'
        config_index = lines.index(config_line)
        lines.insert(config_index + 1, '    cfg["runtime"]["hard_deadline_seconds"]=900; cfg["runtime"]["finalization_margin_seconds"]=120; config=work/"d1_smoke_config.json"; config.write_text(json.dumps(cfg,sort_keys=True))')
        lines = [line.replace('"D1_RELEASE_COMPLETE"', '"D1_SMOKE_COMPLETE"') for line in lines]
    else:
        pass
    lines.append("_d1_main()")
