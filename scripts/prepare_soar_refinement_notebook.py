...
output = Path("/kaggle/working/artifacts/__ARTIFACT_NAME__")
...
command = [sys.executable, str(script), "--challenge-path", str(challenge), "--input-root", str(input_root), "--calibration", str(calibration), "--output-root", str(output), "--task-id", "d4c90558", "--task-id", "67a423a3", "--initial-samples", "__INITIAL_SAMPLES__", "--refinement-rounds", "__REFINEMENT_ROUNDS__", "--refinements-per-round", "__REFINEMENTS_PER_ROUND__"]
...
'''
    source = (source.replace("__ARTIFACT_NAME__", args.artifact_name)
                    .replace("__INITIAL_SAMPLES__", str(args.initial_samples))
                    .replace("__REFINEMENT_ROUNDS__", str(args.refinement_rounds))
                    .replace("__REFINEMENTS_PER_ROUND__", str(args.refinements_per_round)))
    notebook = {
