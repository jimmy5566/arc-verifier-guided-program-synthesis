...
def _notebook(dataset_slug: str, source_commit: str) -> str:
    return "\n".join([
        ...
        'ptxas=Path("/usr/local/cuda-12.5/bin/ptxas"); out=work / "artifacts" / "reference_ttt_production"; out.mkdir(parents=True, exist_ok=True)',
        'if not is_competition_rerun:',
        '    dummy={task_id:[{"attempt_1":[[0]],"attempt_2":[[0]]} for _ in outputs] for task_id,outputs in json.loads(sample.read_text()).items()}',
        '    (work / "submission.json").write_text(json.dumps(dummy,indent=2)+"\\n")',
        '    fast={"event":"FAST_COMMIT_MODE","competition_rerun_detected":False,"model_loaded":False,"gpu_workers_started":False,"submission_kind":"STRUCTURAL_DUMMY_ONLY","task_count":len(dummy),"test_output_count":sum(len(row) for row in dummy.values()),"source_commit":"' + source_commit + '"}',
        '    (out / "FAST_COMMIT_PROVENANCE.json").write_text(json.dumps(fast,indent=2,sort_keys=True)+"\\n")',
        '    print(json.dumps(fast,sort_keys=True),flush=True)',
        '    raise SystemExit(0)',
        ...
        'print(json.dumps({"event":"COMPETITION_RERUN_MODE","competition_rerun_detected":True,"full_reference_ttt_production":True,"source_commit":"' + source_commit + '","gpus":gpus,"ptxas":str(ptxas),"internet":False,"solutions_opened":False},sort_keys=True),flush=True)',
        'shutil.copy2(frozen / "reference_ttt_production_manifest.json", out / "manifest.json"); shutil.copy2(frozen / "reference_ttt_config_frozen.json", out / "config.json")',
