"""Target-blind G1 generation for Smoke12 hybrid representation alignment."""
...
from inference.nvarc_hybrid_representation import HybridRepresentationView
...
FROZEN_STATUS = "SMOKE12_HYBRID_REPRESENTATION_CANDIDATES_FROZEN_BEFORE_EXACT_SCORING"
...
def _identity(manifest: dict[str, Any], variants: dict[str, Any], config: dict[str, Any]) -> str:
    return _hash({"task_ids_hash": manifest["task_ids_hash"], "variant_config_hash": variants["config_hash"], "reference_config": config})
...
def _generate_hybrid(*, model: Any, tokenizer: Any, task: Any, raw_views: list[dict[str, Any]], config: dict[str, Any]) -> tuple[list[dict[str, Any]], int, float]:
    import torch
    ...
    views = [HybridRepresentationView.from_dict(raw) for raw in raw_views]
    if len(views) != 8 or [view.slot for view in views] != list(range(8)):
        raise ValueError("hybrid contract requires eight ordered views")
    if sum(view.kind == "geometry" for view in views) != 6 or sum(view.kind == "reference_like" for view in views) != 2:
        raise ValueError("hybrid contract requires six geometry plus two reference-like views")
    if views[0].augmentation.to_dict() != {"geometry": "identity", "color_offset": 0, "pair_order": "canonical"}:
        raise ValueError("hybrid contract must preserve canonical identity")
    transformed = [view.transform_task(task) for view in views]
    prefixes = [native_training_message_prefix(item) for item in transformed]
    FastLanguageModel.for_inference(model)
    raw: list[dict[str, Any]] = []; invalid = 0; started = time.perf_counter()
    for view, transformed_task, prefix in zip(views, transformed, prefixes, strict=True):
        outputs: list[list[list[int]] | None] = []; tokens = 0; elapsed = 0.0
        for test_index, example in enumerate(transformed_task.test):
            ...
            seed = task_seed(task.task_id, int(config["seed"]), f"{view.to_dict()['seed_namespace']}:{test_index}")
            torch.manual_seed(seed); torch.cuda.manual_seed_all(seed)
            begun = time.perf_counter()
            with torch.inference_mode():
                generated = model.generate(**{name: value.to(model.device) for name, value in encoded.items()}, max_new_tokens=int(config["max_new_tokens"]), do_sample=False, eos_token_id=tokenizer.eos_token_id, pad_token_id=tokenizer.pad_token_id)
            suffix = generated[0, prompt_tokens:].detach().cpu()
            parsed = parse_native_grid(tokenizer.decode(suffix, skip_special_tokens=True))
            outputs.append(None if parsed is None else view.inverse_grid(parsed))
            tokens += int(suffix.shape[-1]); elapsed += time.perf_counter() - begun
            del encoded, generated, suffix
        if any(output is None for output in outputs):
            invalid += 1
            continue
        raw.append({"prediction": [output for output in outputs if output is not None], "representation": view.to_dict(), "completion_tokens": tokens, "generation_seconds": elapsed})
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                candidates, invalid, generation_seconds = _generate_hybrid(model=model, tokenizer=tokenizer, task=tasks[task_id], raw_views=list(variants[task_id]), config=config)
                ...
                record = {"task_id": task_id, "worker_id": worker_id, "physical_gpu_id": worker_id, "status": "SUCCESS" if candidates else "NO_VALID_NATIVE_CANDIDATE", "candidates": candidates, "generated_candidate_count": 8, "unique_candidate_count": len(candidates), "invalid_candidate_count": invalid, "ttt": ttt, "ttt_seconds": float(ttt["seconds"]), "generation_seconds": generation_seconds, "per_output_evidence": evidence, "elapsed_seconds": time.perf_counter() - began, "peak_allocated_vram_mb": int(torch.cuda.max_memory_allocated() / (1024 * 1024)), "adapter_reset_success": reset_ok, "tokenizer": tokenizer_metadata}
...
def main() -> None:
    ...
    if args.output.exists():
        raise FileExistsError("refusing to overwrite frozen Smoke12 hybrid candidates")
    manifest, variants, config = _read(args.manifest), _read(args.variant_config), _read(args.reference_config); task_ids = list(manifest.get("task_ids", ()))
    required = {"rank": 256, "alpha": 32, "ttt_steps": 24, "generation_augmentation_count": 8}
    if manifest.get("status") != "SMOKE12_HYBRID_REPRESENTATION_COHORT_FROZEN" or len(task_ids) != 12 or manifest.get("task_ids_hash") != _task_hash(task_ids):
        raise ValueError("invalid frozen Smoke12 hybrid cohort")
    if variants.get("task_ids") != task_ids or variants.get("config_hash") != manifest.get("variant_config_hash") or variants.get("generation_count") != 8 or set(variants.get("tasks", ())) != set(task_ids):
        raise ValueError("invalid frozen hybrid views")
    ...
    identity = _identity(manifest, variants, config); args.checkpoint_dir.mkdir(parents=True, exist_ok=True); (args.checkpoint_dir / "tasks").mkdir(exist_ok=True)
    ...
    records = {task_id: record for task_id, record in resumed.items() if record is not None}; unfinished = [task_id for task_id in task_ids if task_id not in records]
    context = get_context("spawn"); work, events, ready, start = context.Queue(), context.Queue(), context.Queue(), context.Event(); paths = {"challenge": str(args.challenge_path), "model": str(args.model_path), "native_config": str(args.native_config_dir), "variants": str(args.variant_config), "checkpoint": str(args.checkpoint_dir)}
    ...
    try:
        ...
        if failures or set(records) != set(task_ids):
            raise RuntimeError(f"Smoke12 hybrid G1 incomplete: {failures}")
    finally:
        start.set()
        for process in processes: process.join(timeout=90)
        for process in processes:
            if process.is_alive(): process.terminate()
    artifact = {"experiment_id": manifest["experiment_id"], "status": FROZEN_STATUS, "protocol": "Train-pair-only rank-256 TTT24 then six frozen original Aug8 geometry views plus two frozen reference-like views, with per-output evidence frozen before exact scoring.", "solutions_opened": False, "task_ids": task_ids, "task_ids_hash": manifest["task_ids_hash"], "identity": identity, "reference_config": config, "variant_config_sha256": hashlib.sha256(args.variant_config.read_bytes()).hexdigest(), "worker_model_loads": loads, "hardware": hardware.to_dict(), "runtime_seconds": time.perf_counter() - began, "records": {task_id: records[task_id] for task_id in task_ids}}
    atomic_write_json(args.output, artifact)
    print(json.dumps({"event": "SMOKE12_HYBRID_CANDIDATES_FROZEN", "task_count": len(records), "raw_generation_count": sum(row["generated_candidate_count"] for row in records.values()), "unique_candidate_count": sum(row["unique_candidate_count"] for row in records.values()), "solutions_opened": False}, sort_keys=True), flush=True)
