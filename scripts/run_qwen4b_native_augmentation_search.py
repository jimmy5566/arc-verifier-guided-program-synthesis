...
def _worker(worker_id: int, task_queue: Any, queue_remaining: Any, task_total: int, augmentation_count: int, challenge_path: str, model_path: str, native_config_dir: str, config: dict[str, Any], records: Any, ready: Any, start: Any, enable_ttt: bool, search_beams: int, checkpoint_dir: str | None, checkpoint_identity: str, config_sha256: str, deadline_unix: float | None, generation_micro_batch_size: int, likelihood_micro_batch_size: int) -> None:
    ...
    try:
        ...
        while True:
            ...
            # Telemetry only: these task-local timestamps are persisted with
            # the atomic checkpoint so strict production finalization can
            # audit start/end/runtime without changing decoding behavior.
            task_started_unix = time.time()
            ...
            for attempt in range(2):
                ...
                try:
                    ...
                    serialized_unique = [item.to_dict() for item in unique]
                    likelihood_by_index = {unique.index(item): float(score) for item, score in ranked}; ranking_indices = rank_indices(feature_rows(serialized_unique, likelihood_by_index)) if unique else {}
                    b_support_started = time.perf_counter()
                    view_scores = candidate_view_scores_many(provider, task, [item["prediction"] for item in serialized_unique], b_support_views, context_window=int(settings["decode"]["context_window"]), batch_size=likelihood_micro_batch_size) if unique else []
                    ...
                    completed_record = {"task_id": task_id, "worker_id": worker_id, "physical_gpu_id": worker_id, "status": "SUCCESS" if ranked else "NO_VALID_NATIVE_CANDIDATE", "baseline_prediction": baseline_prediction[0] if baseline_prediction and len(baseline_prediction) == 1 else baseline_prediction, "candidates": serialized_unique, "ranked_candidate_indices": [unique.index(item) for item, _score in ranked], "candidate_scores": [score for _item, score in ranked], "ranking_indices": ranking_indices, "ranked_prediction": ([[list(row) for row in grid] for grid in ranked[0][0].prediction][0] if len(ranked[0][0].prediction) == 1 else [[list(row) for row in grid] for grid in ranked[0][0].prediction]) if ranked else None, "generated_candidate_count": generated_count, "unique_candidate_count": len(unique), "invalid_candidate_count": invalid, "completion_tokens": token_total, "generation_seconds": generation_seconds, "generation_wall_seconds": generation_wall_seconds, "original_likelihood_seconds": original_likelihood_seconds, "b_support_scoring_seconds": b_support_scoring_seconds, "b_support_view_spec": [view.to_dict() for view in b_support_views], "b_support_evidence": b_support_evidence, "model_vram_mb": provider.load_metadata.get("model_vram_mb"), "peak_allocated_vram_mb": int(torch.cuda.max_memory_allocated() / (1024 * 1024)), "retry_count": attempt, "ttt": ttt_metrics, "task_started_unix": task_started_unix, "task_completed_unix": time.time(), "attempt_elapsed_seconds": time.perf_counter() - attempt_started, "elapsed_seconds": time.perf_counter() - task_started_monotonic, "gpu_samples": gpu_samples, "seed_policy": "sha256(global_seed, task_id, augmentation, test_index)", "execution_optimization": {"generation_micro_batch_size": generation_micro_batch_size, "likelihood_micro_batch_size": likelihood_micro_batch_size, "inline_b_support_scoring": True}}
                    break
