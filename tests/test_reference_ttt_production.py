...
def test_production_runner_and_notebook_use_fast_save_and_full_competition_rerun() -> None:
    ...
    for forbidden in ("allow-deadline-partial", "identity fallback", "beam2", "DFS"):
        assert forbidden.lower() not in combined.lower()
    assert "KAGGLE_IS_COMPETITION_RERUN" in builder
    assert "FAST_COMMIT_MODE" in builder and "COMPETITION_RERUN_MODE" in builder
    assert "if not is_competition_rerun:" in builder
    assert '"model_loaded":False' in builder and '"gpu_workers_started":False' in builder
